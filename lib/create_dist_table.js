...
function create_distance_table(opts,cb){
    ...
    var vds_geoview =
            ...
            'vv.version,' +
            'vf.freeway_id,' +
            'newctmlmap.canonical_direction(vf.freeway_dir) as direction,' +
    ...
    var wim_sites =
        ...
        +',geom'
            + ' from wim_geoview w where site_no < 800 and '+
            'site_no in (' +
            sites.join(',') +
            '))'
               ...
               ,function(err,client,clientdone){
                   ...
                   client.query(query,function(e,r){
                       ...
                       r.rows.forEach(function(row){
                           ...
                           if(hashmap[cdbwimid] === undefined){
                               hashmap[cdbwimid] = [row]
                           }else{
                               hashmap[cdbwimid].push(row)
                           }
                       })
                       cb(null,hashmap)
                       clientdone()
